...
//*      * 12/12/2020 * Get 6 least significative bits from I2C bus to select the band  *
...
float tempFanActivation;
float previousTemp_fanActivation;
...
void setup() {
  ...
  u8g2.firstPage();
  do {
    u8g2.drawStr(5,25,"PA100 Ctrl v3.0");
    u8g2.drawStr(5,53,"      EA3IGT");
  } while ( u8g2.nextPage() );
  ...
  //Get stored Band from EEPROM Byte 0
...
{
  int PWMDuty;
  int Times = 30;                                   //Average 30 times to be more stable
  float Temperature = 0;
  for(int i = 0; i < Times; i++) {
    ...
    T = T - 273.15 + 1;                             //+1 is my personal adjust to be more precise
  ...
  PWMDuty = int((myTemp - tempFanActivation)*15);   //Calculate PWMDuty (increase to get more aggresive fan)
